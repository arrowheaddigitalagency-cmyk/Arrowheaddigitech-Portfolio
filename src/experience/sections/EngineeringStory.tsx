...
import { ArrowUpRight, Atom, Blocks, Bot, Braces, BrainCircuit, CircuitBoard, Cloud, Code2, Container, Database, GitBranch, Globe2, Layers3, Linkedin, Megaphone, Network, Server, ShieldCheck, ShoppingBag, Smartphone, Target, TrendingUp, Workflow } from 'lucide-react';
...
const narratives=[
 ...
 {label:'THE CONNECTION',text:'A website is only the beginning. We connect your platform, mobile apps, dashboards and automations around a shared foundation — so your customers get the same experience on every screen.'},
...
const stackGroups=[
 ...
 {title:'MARKETING',icon:Megaphone,items:[{name:'Google Ads',icon:Target},{name:'Meta Ads',icon:Megaphone},{name:'Traffic Guard',icon:ShieldCheck},{name:'Growth analytics',icon:TrendingUp}]},
];
export default function EngineeringStory(){
 ...
 useEffect(()=>{
  ...
  // Defer WebGL machine until the journey is near — kills project→engineer lag.
  ...
  media.add('(prefers-reduced-motion: no-preference)',()=>{
   ...
   const scrubMain=touch?1.05:.7;
   const scrubText=touch?.8:.4;
  ...
  });return()=>{media.revert();observer.disconnect();machineIo?.disconnect();};
 },[]);
 ...
 return <div className="refined-sections" ref={root}>
  ...
  <section className="platform-narrative" id="platforms"><div className="narrative-heading"><SectionLabel number="04">APPS &amp; PLATFORMS</SectionLabel><h2>Made for your business.<br/><span>Ready for what’s next.</span></h2></div>
